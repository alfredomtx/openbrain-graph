...
function showPanel(entity, graph, nodeId) {
  ...
  if (entityLinks.length) {
    ...
    linkedThoughts.forEach(t => {
      html += `<div class="linked-thought">${truncate(t.content, 120)}</div>`;
    });
    if (entityLinks.length > 5) html += `<div style="color:#555;font-size:11px;margin-top:4px">+${entityLinks.length - 5} more</div>`;
    html += `</div>`;
  }
...
function updateStats() {
  ...
  document.getElementById('stat-types').textContent = Object.entries(typeCounts)
